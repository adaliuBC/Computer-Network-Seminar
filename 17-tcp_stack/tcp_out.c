...
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len) 
{
    ...
    if(!resend) {
        ...
        //TODO
        struct tcp_packet *unacked_pkt = (struct tcp_packet *)malloc(sizeof(struct tcp_packet));
        unacked_pkt->packet = (char *)malloc(len);
        unacked_pkt->len = len;
        memcpy(unacked_pkt->packet, packet, len);
        list_add_tail(&(unacked_pkt->list), &(tsk->send_buf));
        printf("SEND_BUF HAVE 1 DATA PKT\n");
        ...
        resend = 0;
        printf("RESEND: SEND BUF NO CHANGE\n");
        if(tsk->retrans_timer.enable==0) {
            tcp_set_retrans_timer(tsk);
        }   
	    ip_send_packet(packet, len);
...
void tcp_send_control_packet(struct tcp_sock *tsk, u8 flags)
{
	...
	if (flags & (TCP_SYN|TCP_FIN)) {
		...
		if (!resend) {
		    struct tcp_packet *unacked_pkt = (struct tcp_packet *)malloc(sizeof(struct tcp_packet));
            unacked_pkt->packet = (char *)malloc(pkt_size);
            unacked_pkt->len = pkt_size;
            printf("send_control-2, %d\n", pkt_size);
            memcpy(unacked_pkt->packet, packet, pkt_size);
            list_add_tail(&(unacked_pkt->list), &(tsk->send_buf));
            printf("SEND_BUF HAVE 1 CONTROL PKT\n");
        } else {
            resend = 0;
        }
        if(tsk->retrans_timer.enable==0) {
